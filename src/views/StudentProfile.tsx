...
import { Save, Camera, Shield, Activity, Star, BookOpen, Hash, Mail, CalendarDays, Pencil, QrCode, X } from 'lucide-react';
...
const StudentProfile = () => {
  ...
  const [studentStats, setStudentStats] = useState({ score: 0, achievements: 0, events: 0, tier: 'Novice' });
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      if (!user) return;
      try {
        const stats = await EngagementService.getUserEngagement(user.id);
        setStudentStats({
          score:        stats.totalXp,
          achievements: stats.achievementsEarned,
          events:       stats.eventsAttended,
          tier:         stats.reputationTier
        });
  ...
  const stats = [
    { label: 'Total XP', value: studentStats.score, icon: Activity, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20' },
        ...
        <div className="w-full lg:w-80 flex-shrink-0 space-y-6">
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-semibold border border-gray-200 dark:border-gray-700">
                <Shield size={14} className="text-blue-500" /> Verified Student
              </div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 text-xs font-semibold border border-emerald-200 dark:border-emerald-800">
                <Star size={14} className="text-emerald-500" /> {studentStats.tier} Tier
              </div>
            </div>

            <button
              onClick={() => setShowIdCard(true)}
              className="mt-6 w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-black dark:bg-white dark:hover:bg-gray-100 dark:text-gray-900 text-white py-3 rounded-xl font-bold shadow-md transition-all active:scale-95"
            >
              <QrCode size={18} /> Digital ID Card
            </button>
          </motion.div>
          ...
          >
            <BadgeShowcase
              userId={user?.id}
              stats={{
                total_xp: studentStats.score,
                events_attended: studentStats.events,
              }}
            />
          </motion.div>
        </div>
        ...
        {showIdCard && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 rounded-3xl p-8 w-full max-w-sm shadow-2xl relative border border-gray-200 dark:border-gray-800 flex flex-col items-center text-center"
            >
              <button
                onClick={() => setShowIdCard(false)}
                className="absolute top-4 right-4 p-2 bg-gray-100 dark:bg-gray-800 text-gray-500 hover:text-gray-900 dark:hover:text-white rounded-full transition-colors"
              >
                <X size={18} />
              </button>
              
              <div className="w-16 h-16 rounded-full overflow-hidden border-4 border-white dark:border-gray-800 shadow-lg -mt-14 mb-4 bg-gray-200">
                 {displayAvatar ? (
                    <img src={displayAvatar} alt="Avatar" className="w-full h-full object-cover" />
                 ) : (
              ...
              <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mt-1 uppercase tracking-widest">{formData.course || 'Course Not Set'}</p>
              
              <div className="my-8 p-4 bg-white rounded-2xl shadow-inner border-2 border-gray-100 inline-block">
                <QRCode 
                  value={user ? AttendanceService.generateStudentQRPayload(user.id) : 'invalid'} 
                  size={180} 
                  level="H"
                />
              </div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 max-w-[250px]">
                Show this QR code to Club Heads at events to log your attendance and earn XP.
              </p>
            </motion.div>
          </motion.div>
        )}
