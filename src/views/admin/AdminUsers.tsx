...
import { Search, Mail, Key, Trash2, Shield, Calendar, Edit2, MoreVertical, Users, X, ArrowRight, RefreshCw } from 'lucide-react';
import { format } from 'date-fns';
import { motion, AnimatePresence } from 'framer-motion';
import { toast } from 'react-toastify';
import { adminUpdateUserPassword } from '../../app/actions/admin';
...
const AdminUsers = () => {
  ...
  const [passwordData, setPasswordData] = useState({ userId: '', email: '', newPassword: '', confirmPassword: '' });
  ...
  const handleUpdatePassword = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Call the server action for a direct overwrite
      const result = await adminUpdateUserPassword(passwordData.userId, passwordData.newPassword);
      ...
      if (!result.success) {
        ...
        toast.success('User password has been modified successfully!');
      }
                ...
                filteredUsers.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      ...
                      <button 
                        onClick={() => {
                          setPasswordData({ userId: u.id, email: u.email, newPassword: '', confirmPassword: '' });
                          setIsPasswordModalOpen(true);
                        }}
                        className="text-gray-400 dark:text-gray-500 hover:text-amber-600 dark:hover:text-amber-400 p-2 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 ml-1"
            ...
            >
              <div className="px-8 py-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">Security Reset</h2>
                </div>
                <button
              ...
              <form onSubmit={handleUpdatePassword} className="p-8 space-y-5">
                <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800/30 flex gap-3">
                  ...
                  <p className="text-xs text-blue-800 dark:text-blue-300 leading-relaxed font-medium">
                    Resetting password for <span className="font-black underline">{passwordData.email}</span>. Use a strong, unique password.
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest">Confirm Password</label>
                    <input 
                      type="password"
                      required
                ...
                <div className="pt-6 flex flex-col gap-3">
                  <button
                    type="submit"
                    disabled={isResetting || !passwordData.newPassword}
                  ...
                  >
                    {isResetting ? (
                      ...
                      <>Overwrite Password <ArrowRight size={18} /></>
                    )}
                  </button>
                  <button
                    type="button"
                    onClick={() => setIsPasswordModalOpen(false)}
                    className="w-full py-3 text-sm font-bold text-gray-500 hover:text-gray-700 transition-all"
                  >
                    Cancel Action
                  </button>
                </div>
