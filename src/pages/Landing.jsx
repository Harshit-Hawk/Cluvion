...
/* ──────────────────────────────────────────────────────────
   MARQUEE
────────────────────────────────────────────────────────── */
const TICKER_WORDS = ['Club Management', 'Event Timelines', 'Leaderboards', 'Achievement Tracking', 'Campus Sync', 'Community Feed', 'Points Engine', 'Role Dashboards', 'Live Feeds', 'Smart Analytics'];
...
export default function Landing() {
  ...
  const features = [
    { icon: Users, label: 'Core Module', title: 'Club Nexus', body: 'Create, join, and manage clubs with real-time member sync, role assignments, and dedicated announcement boards.', accentClass: 'bg-indigo-500' },
    { icon: Calendar, label: 'Event Pipeline', title: 'Chronos Events', body: 'A living campus timeline. Plan events, send invites, track RSVPs, and get attendance reminders — all automated.', accentClass: 'bg-purple-500' },
    { icon: Trophy, label: 'Prestige Engine', title: 'Aura Leaderboard', body: 'Every action earns points. Compete in department and campus-wide leaderboards, rising through achievement tiers.', accentClass: 'bg-amber-500' },
    { icon: TrendingUp, label: 'Activity Feed', title: 'Personal Timeline', body: 'Your engagement history, visualised. Every club joined, event attended, and achievement unlocked in one scroll.', accentClass: 'bg-emerald-500' },
    { icon: Shield, label: 'Access Control', title: 'Role Vaults', body: 'Three roles, three precision-tuned dashboards. Students, Club Heads, and Admins each get exactly what they need.', accentClass: 'bg-rose-500' },
    { icon: BookOpen, label: 'Discovery Engine', title: 'Campus Explorer', body: 'Surface every opportunity at your institution. Filter clubs by interest, find events by date, and discover your people.', accentClass: 'bg-sky-500' },
  ];

  const steps = [
    { number: '01', title: 'Enter your college domain', body: 'Type your institution\'s email domain (e.g. gateway.edu.in) to verify you\'re on the right campus network.' },
    { number: '02', title: 'Create or sign into your account', body: 'Register with your college email in under 30 seconds. No extra apps, no waiting for approvals.' },
    { number: '03', title: 'Discover & join clubs', body: 'Browse every active club at your institution. Tap to join instantly and start getting updates in your feed.' },
    { number: '04', title: 'Build your campus legacy', body: 'Attend events, earn achievement points, and climb the leaderboard. Your Cluvion profile tells your whole story.' },
  ];
    ...
    <div className={`min-h-screen overflow-x-hidden font-sans antialiased transition-colors duration-500 ${bg}`} onMouseMove={handleMouse}>
      ...
      <nav className={`fixed top-0 inset-x-0 z-[100] flex items-center justify-between px-6 lg:px-12 h-[70px] border-b backdrop-blur-xl transition-colors duration-500 ${navBg}`}>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="flex items-center gap-3">
          ...
          <button onClick={() => navigate('/welcome')} className={`hidden sm:block text-sm font-semibold transition-colors ${dark ? 'text-white/40 hover:text-white' : 'text-gray-500 hover:text-gray-900'}`}>Sign In</button>
          <button onClick={() => navigate('/welcome')} className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold px-5 py-2.5 rounded-full transition-all duration-300 shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 group">
      ...
      <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden px-6 pt-20">
        ...
        <motion.div style={{ y: heroY, opacity: heroO }} className="relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center text-center gap-9">
          ...
          <motion.p initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.28 }}
            className={`max-w-2xl text-lg sm:text-xl font-medium leading-relaxed ${dark ? 'text-white/50' : 'text-gray-500'}`}>
            The all-in-one platform where campus life comes alive. Discover clubs, own the leaderboard, track every achievement, and connect with your academic community — from a single, beautifully designed portal.
          </motion.p>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <Reveal delay={0.1}>
            <blockquote className={`text-2xl md:text-4xl font-bold leading-snug tracking-tight ${dark ? 'text-white' : 'text-gray-900'}`}>
              "Your student years are your most formative. Cluvion makes sure <em className="not-italic text-indigo-500">every moment counts</em> — every club, event, and achievement woven into your story."
            </blockquote>
          </Reveal>
          <Reveal delay={0.2}>
            ...
            <Reveal delay={0} className="md:col-span-2">
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[['indigo', 'Points'], ['purple', 'Clubs'], ['emerald', 'Events']].map(([c, l]) => (
            ...
            <div className="flex flex-col gap-5">
              ...
              <Reveal delay={0.2}>
                <div className={`rounded-3xl p-6 border ${dark ? 'bg-[#0e0e12] border-white/5' : 'bg-white border-gray-100 shadow-sm'}`}>
                  <MessageSquare size={28} className="text-purple-500 mb-4" strokeWidth={1.5} />
                  <h4 className={`font-black text-lg mb-1 ${dark ? 'text-white' : 'text-gray-900'}`}>Club Feed</h4>
                  <p className={`text-sm ${dark ? 'text-white/40' : 'text-gray-500'}`}>Every announcement from your clubs, in one live feed.</p>
                </div>
              </Reveal>
            </div>
          </div>
